...
class FileUploaded extends AFile {
	...
	public function getBaseDir() { return $this->m_config->getTmpDir(); }

	public function getNewName() { return $this->m_newName; }
    ...
    public function checkForConflicts($dir) {
		...
		if ($file->exists())
            $this->m_confilictsErrors[] = Message::createMessage(Message::FILE_ALREADY_EXISTS, $file->getName());

		if ($file->isImage()) {
            $fileOriginal = $file->getFileOriginal();
			if ($fileOriginal->exists())
                $this->m_confilictsErrors[] = Message::createMessage(Message::FILE_ALREADY_EXISTS, $fileOriginal->getName());

			$filePreview = $file->getFilePreview();
			if ($filePreview->exists())
                $this->m_confilictsErrors[] = Message::createMessage(Message::FILE_ALREADY_EXISTS, $filePreview->getName());
		}
	...
	public function rehost($url) {
        $dUrl = URLDownloader::download($url, $this->getBaseDir() . "/" . $this->getDir());
